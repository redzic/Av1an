...
impl<'a> Aom<'a> {
  pub fn new(
    ...
    Self(TwoPassEncoder::new(
      ...
      |chunk_index, encoder_args, output_file| {
      ...
      |first_pass_file, encoder_args, output_file| {
        dbg!(output_file);
        let mut second_pass = Command::new("aomenc");
        second_pass.args(&["-", "--passes=2", "--pass=2"]);
        second_pass.args(encoder_args);
        second_pass.arg(format!("--fpf={}", "fpf0.log"));
        second_pass.arg("-o");
        second_pass.arg(output_file);
        second_pass
      },
      progress_sender,
      encoder_args,
...
const IRRELEVANT_CHARS: &'static str = "Pass -/- frame  ---/";
const FIRST_PASS_CHARS: &'static str = "Pass 1/2 frame";
